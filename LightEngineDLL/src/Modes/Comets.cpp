/*
  ...
  Displays comet-like trails with fading brightness.
...
void renderComets(HSV* leds, int numLeds, const RenderContext* ctx) {
    ...
    for (int line = 0; line < lines; line++) {
        for (int led = ledStart; led < ledStart + ledLength; led++) {
            ...
            int localPos = led - ledStart;
            int fadeFactor = ledLength - localPos;
            int brightness = (bright * fadeFactor) / ledLength;
            
            leds[idx].h = static_cast<uint8_t>(hue);
            leds[idx].s = static_cast<uint8_t>(sat);
            leds[idx].v = static_cast<uint8_t>(brightness);
        }
    }
}
